...
function initializeEditDeckName() {
  ...
  console.log("Initializing edit deck name functionality", {
    editBtn,
    deckInput,
    deckTitle,
  });
...
$(document).ready(function () {
  ...
  function checkForDueCards() {
      ...
      console.log(
        ...
        dueCards.map((card) => {
          ...
          return {
            ...
            dueTime: cardData?.due
              ? new Date(cardData.due).toLocaleString()
              : "unknown",
    ...
    const newlyDueCards = dueCards.filter((dueCard) => {
      ...
      return !flashcards.some((flashcard) => {
        const flashcardId =
          flashcard.id || `card_${originalDeckCards.indexOf(flashcard)}`;
        return flashcardId === cardId;
      });
    });

    console.log(
      `Found ${newlyDueCards.length} newly due cards to add to session`
    );
    ...
    if (newlyDueCards.length > 0) {
      ...
      // Show notification
      showSuccessMessage(
        `🔄 ${newlyDueCards.length} card(s) are now due for review and have been added to your session!`
      );
  ...
  // Note: Auth state changes are handled in the DOMContentLoaded listener above
  // Authentication is now initialized via initializeAuthSystem() in auth-modal.js
  ...
  function updateAuthUI() {
    ...
    `);
      ...
      $(document).on("click", "#googleLoginBtn", async function () {
        console.log(
          "[APP.JS] Google login button clicked - delegating to auth-modal.js"
        );
    ...
    `
        ...
        deckList.querySelectorAll(".delete-deck").forEach((button) => {
          button.addEventListener("click", async function () {
            ...
            try {
              ...
              showNotification(
                "Failed to delete deck: " + error.message,
                "error"
              );
            } finally {
              showLoading(false);
        ...
        deckList.querySelectorAll(".export-deck").forEach((button) => {
          button.addEventListener("click", async function () {
            ...
            try {
              ...
              showNotification(
                "Failed to export deck: " + error.message,
                "error"
              );
            }
          });
      ...
      function initializeFlashcardControls() {
        ...
        $("#showAnswerBtn").on("click", function (e) {
          ...
          if (
            currentCardIndex === 0 &&
            !localStorage.getItem("seenSpacedRepetitionTip")
          ) {
            setTimeout(() => {
        ...
        $(document)
          .off("keydown.performance")
          .on("keydown.performance", function (e) {
            console.log(
              `Key pressed: ${e.key}, buttons visible: ${$(
                ".performance-buttons"
              ).is(":visible")}, display: ${$(".performance-buttons").css(
                "display"
              )}`
            );
            ...
            if (
              !$(".performance-buttons").is(":visible") ||
              $(".performance-buttons").css("display") === "none"
            ) {
              return;
            }
            ...
            if (keyMap[e.key]) {
              e.preventDefault();
              console.log(
                `Keyboard shortcut ${e.key} pressed for performance ${
                  keyMap[e.key]
                }`
              );
              ...
              const button = $(
                `.performance-buttons button[data-performance="${
                  keyMap[e.key]
                }"]`
              );
        ...
        $("#flashcard").on("click", function (e) {
          ...
          console.log(
            "flashcardHandled flag:",
            e.originalEvent
              ? e.originalEvent.flashcardHandled
              : "no originalEvent"
          );
          ...
          if (e.originalEvent && e.originalEvent.flashcardHandled) {
            console.log(
              "Custom flip handler already processed this click, skipping app.js handler"
            );
            return;
          }
          ...
          if (
            $(e.target).closest(
              ".performance-buttons, .btn-pronounce, .navigation-buttons"
            ).length > 0
          ) {
            return;
          }
      ...
      function updateFlashcard() {
        ...
        frontPronounceBtn.setAttribute(
          "data-text",
        ...
        console.log(
          "app.js updateFlashcard reset - flashcard classes:",
          $("#flashcard")[0] ? $("#flashcard")[0].className : "not found"
        );
      ...
      async function processDocxFile(content, fileType) {
        try {
          ...
          if (fileType === "application/msword") {
            ...
            throw new Error(
              "Legacy DOC files require conversion to DOCX for full support. Please save your document as DOCX format."
            );
          }
      ...
      async function processPowerPointFile(content, fileType) {
        try {
          ...
          if (fileType === "application/vnd.ms-powerpoint") {
            throw new Error(
              "Legacy PPT files require conversion to PPTX for full support. Please save your presentation as PPTX format."
            );
          }
          ...
          if (!extractedText.trim()) {
            throw new Error(
              "No text content could be extracted from this presentation. Make sure the slides contain text."
            );
          }
      ...
      async function processSpreadsheetFile(content, fileType) {
        try {
          ...
          if (fileType === "application/vnd.ms-excel") {
            throw new Error(
              "Legacy XLS files require conversion to XLSX for full support. Please save your spreadsheet as XLSX format."
            );
          }
          ...
          if (!extractedText.trim()) {
            throw new Error(
              "No data could be extracted from this spreadsheet."
            );
          }
      ...
      function validateAndProcessFile(file) {
        ...
        const maxSize = file.type.includes("presentation")
          ? 15 * 1024 * 1024
          : 10 * 1024 * 1024;
        ...
        if (file.size > maxSize) {
          showErrorMessage(
            `File size (${sizeMB}MB) exceeds the ${maxSizeMB}MB limit`
          );
          return;
        }
        ...
        if (
          ...
          showSuccessMessage(
            `Legacy ${file.name
              .split(".")
              .pop()
              .toUpperCase()} format detected. For best results, please save as modern format (DOCX, PPTX, XLSX) if possible.`
          );
        }
      ...
      function generateTagsFromContent(front, back) {
        ...
        const specificTerms = importantWords
          .slice(0, 2)
      ...
      function validateAIResponse(cards) {
        return cards.every((card) => {
          ...
          if (card.front === card.back) return false;
          if (
            card.front.includes("undefined") ||
            card.back.includes("undefined")
          )
            return false;
      ...
      $(document).on("click", "#cancelSubBtn", async function () {
        ...
        const daysSinceStart =
          (now - subscriptionStart) / (1000 * 60 * 60 * 24);
    ...
    `;
      ...
      function updateStudyMenuStats() {
        if (flashcards.length === 0) {
          document.getElementById("dueCardsCount").textContent =
            "No cards loaded";
          document.getElementById("unfamiliarCardsCount").textContent =
            "No cards loaded";
          document.getElementById("allCardsCount").textContent =
            "No cards loaded";
          return;
        }
    ...
    `;
        ...
        modal
          .querySelector("#startReviewSession")
          .addEventListener("click", () => {
            modal.remove();
            showStudyMenu();
          });
        ...
        modal.querySelector("#viewAllDecks").addEventListener("click", () => {
          ...
          document
            .querySelector('.nav-tabs .tab-btn[data-tab="decks"]')
            ?.click();
        });
      ...
      async function processImageForOcclusion(imageData, options = {}) {
        try {
          ...
          if (
            !occlusionData ||
            !occlusionData.image ||
            !occlusionData.occlusions ||
            occlusionData.occlusions.length === 0
          ) {
            showErrorMessage(
              "Please create at least one occlusion area before generating flashcards."
            );
            return;
          }
          ...
          formData.append(
            "occlusions",
            JSON.stringify(occlusionData.occlusions)
          );
          ...
          formData.append(
            "cardCount",
            options.cardCount || occlusionData.occlusions.length
          );
          ...
          const flashcards = occlusionData.occlusions.map(
            (occlusion, index) => ({
          ...
          showSuccessMessage(
            `Created ${flashcards.length} image occlusion flashcards successfully!`
          );
        } catch (error) {
  ...
  renderTagFilters(categoryTags, activeCategory) {
    ...
    tagFilterContainer.innerHTML = `
      <div class="tag-filter-header">
        ...
        <button class="btn btn-link btn-small clear-tags" ${
          this.selectedTags.size === 0 ? 'style="display: none;"' : ""
        }>
    ...
    container.querySelectorAll(".tag-filter-chip").forEach((chip) => {
      chip.addEventListener("click", () => {
        ...
        clearBtn.style.display =
          this.selectedTags.size > 0 ? "inline-flex" : "none";
...
async function getSharedDecks() {
  try {
    ...
    return [
      {
        id: "top_100_nec_2026_questions",
        name: "🎯 FREE: Top 100 NEC 2026 Questions",
        description:
          "The most frequently asked NEC 2026 questions that appear on journeyman exams. Master these and you're 80% there!",
        category: "electrician",
        level: "all",
        tags: [
          "NEC",
        ...
        cards: [
          {
            front:
              "What is the minimum working space depth required for 480V equipment when exposed live parts are on one side?",
            back: "4 feet",
            explanation:
              "NEC 2026 Article 110.26(A)(1) requires 4 feet minimum working space depth for equipment operating at 480V when exposed live parts are on one side.",
          },
          {
            front:
              "GFCI protection is required for all ___ receptacles in bathrooms.",
            back: "125V, single-phase, 15- and 20-ampere",
            explanation:
              "Article 210.8(A)(1) requires GFCI protection for all 125V, single-phase, 15- and 20-ampere receptacles in bathrooms.",
          },
        ],
      },
      {
...
function handlePerformanceAnswer(answer) {
  ...
  console.log(
    ...
    answerLabels[answer] || `Unknown (${answer})`
  );
  ...
  console.log("🧠 BEFORE Processing - Spaced Repetition State:", {
    ...
    due: spacedRepData?.due
      ? new Date(spacedRepData.due).toLocaleString()
      : "never",
  ...
  console.log(
    "⏱️ Response Time:",
    responseTime ? `${responseTime.toFixed(2)}s` : "not measured"
  );
  ...
  if (!isActiveStudySession && flashcards.length > 0) {
    console.warn(
      "⚠️ SESSION TRACKING NOT ACTIVE - Auto-starting session tracking"
    );
    isActiveStudySession = true;
    originalDeckCards = [...flashcards];
    startDueCardChecking();
  ...
  if (
    ...
    window.spacedRepetitionInstance
  ) {
    const cardId = card.id || `card_${currentCardIndex}`;
    try {
      ...
      const updatedSpacedRepData =
        window.spacedRepetitionInstance.getCardData(cardId);
      console.log("🧠 AFTER Processing - Spaced Repetition State:", {
        ...
        due: updatedSpacedRepData?.due
          ? new Date(updatedSpacedRepData.due).toLocaleString()
          : "never",
        nextReviewIn: updatedSpacedRepData?.due
          ? Math.round((updatedSpacedRepData.due - Date.now()) / 1000) + "s"
          : "never",
        ...
        willShowAgainSoon:
          updatedSpacedRepData?.due &&
          updatedSpacedRepData.due - Date.now() < 300000, // within 5 minutes
      });
      ...
      if (updatedSpacedRepData?.state === "learning") {
        console.log("📚 LEARNING CARD DETECTED:", {
          ...
          willReappearIn: updatedSpacedRepData?.due
            ? Math.round((updatedSpacedRepData.due - Date.now()) / 1000) +
              " seconds"
            : "unknown",
          isWithinSession:
            updatedSpacedRepData?.due &&
            updatedSpacedRepData.due - Date.now() < 300000, // 5 minutes
        });
      }
      ...
      if (
        updatedSpacedRepData?.due &&
        updatedSpacedRepData.due - Date.now() < 300000
      ) {
        console.log(
          "⚠️ CARD WILL REAPPEAR SOON - Will be picked up by due card checker"
        );
      }
    } catch (error) {
...
function updateDeckTags() {
  ...
  if (tagCategories.difficulty.size > 0) {
    ...
    const mostCommonDifficulty = Object.entries(difficultyCount).sort(
      ([, a], [, b]) => b - a
    )[0];
...
function generateTagsFromContent(front, back) {
  ...
  for (const [subject, data] of Object.entries(subjectCategories)) {
    ...
    if (matches > maxMatches) {
      ...
      const subcategoryMatches = data.subcategories.map((subcat) => ({
        subcat,
        matches: content.split(/\s+/).filter((word) => word.includes(subcat))
          .length,
      }));
      subjectSubcategory = subcategoryMatches.sort(
  ...
  const importantTerms = words
    .filter(
      (word) =>
        word.length > 3 && !stopWords.has(word) && /^[a-zA-Z]+$/.test(word) // Only pure words, no numbers or special chars
    )
